...
const ResultPanel = ({ data, onClose }) => {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const addText = (text, size, isBold = false, color = '#ffffff') => {
      ...
      addText(`Total Cost: $${totalCost.toFixed(2)}  |  Agents Hired: ${data.subtasks?.length || 0}`, 10);
      doc.setTextColor(255, 255, 255);
      y += 4;
  ...
  const exportDOCX = async () => {
    ...
    try {
      ...
      const addSeparator = () => {
        children.push(
          new Paragraph({
            ...
            borders: { bottom: { style: BorderStyle.SINGLE, size: 1, color: '6366F1' } },
          })
        );
      };
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'TASKFORCE - MISSION REPORT', bold: true, size: 32, color: '6366F1' })],
      ...
      // Metadata
      addBody(`Generated: ${formatDate()}`, { color: '888888', size: 20 });
      addBody(`Status: ${data.status || 'COMPLETED'}`, { color: '888888', size: 20 });
      addBody(`Total Cost: $${totalCost.toFixed(2)}  |  Agents Hired: ${data.subtasks?.length || 0}`, { color: '888888', size: 20 });
      ...
      if (data.subtasks) {
        data.subtasks.forEach((st, idx) => {
          children.push(new Paragraph({
            children: [new TextRun({ text: `${idx + 1}. ${st.agent}`, bold: true, size: 24, color: '6366F1' })],
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'Generated by TaskForce - Autonomous Agent Economy', size: 16, color: '666666', italics: true })],
        alignment: AlignmentType.CENTER,
      }));
